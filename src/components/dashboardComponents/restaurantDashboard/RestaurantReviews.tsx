...
import { getRestaurantReviewsApi, IReviewSummary } from "@/lib/api/review";
...
export default function RestaurantReviews() {
  ...
  const loadData = useCallback(async () => {
    setRefreshing(true);
    try {
      const resData = await getMyRestaurantProfile(user?.id || "", user?.email || "");
      const rest = resData?.success && resData?.data ? resData.data : null;
      setRestaurant(rest);
  ...
  const avgRating = data?.avgRating || restaurant?.rating || 0;
  const totalReviews = data?.totalReviews || restaurant?.reviewCount || 0;
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4 space-y-8 animate-in fade-in duration-200">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/20 backdrop-blur-md text-xs font-bold uppercase tracking-wider text-white border border-white/30">
              <Store className="w-3.5 h-3.5 text-amber-200" /> Restaurant Reputation
            </div>
            <h1 className="text-2xl md:text-4xl font-extrabold tracking-tight text-white">
              My Ratings & Reviews
            </h1>
            <p className="text-orange-100 text-sm max-w-lg">
              {restaurant?.name || "Your Restaurant"} • Real-time customer ratings, star breakdown, and reputation stats.
            </p>
          </div>
      ...
      {/* 📊 Overview Cards (Mirrored from Rider Reviews) */}
        ...
        <div className="p-6 rounded-3xl bg-white border border-orange-100/80 shadow-xs flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 font-medium mt-1">
              Based on {totalReviews} store ratings
            </p>
          </div>
        </div>
      ...
      {/* 📈 Star Rating Distribution Bar (Mirrored from Rider Reviews) */}
      ...
      <div className="p-6 rounded-3xl bg-orange-50/70 border border-orange-200/80 text-gray-800 text-xs font-semibold flex items-center gap-3.5 shadow-xs">
        ...
        <p className="leading-relaxed">
          Your overall store and recipe average rating scores are updated in real-time from verified post-delivery customer reviews. Individual feedback entries are monitored directly by the FoodFlow Admin quality control team.
        </p>
      </div>
    </div>
  );
