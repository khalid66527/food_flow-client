...
export default function DashboardSideBar() {
  ...
  const userName = session?.user?.name || "John Customer";
  const userEmail = session?.user?.email || "customer@foodflow.com";
  ...
  return (
    <>
      ...
      <div className="md:hidden sticky top-0 z-40 flex items-center justify-between px-4 py-3 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-xs">
        ...
        <Link
          href="/dashboard/customer/cart"
          className="p-2 rounded-xl text-[#FF6B35] bg-orange-50 relative"
      ...
      >
        ...
        {/* Footer with Loyalty & User Account Info */}
          ...
          {!isCollapsed && (
            <div className="p-2.5 rounded-xl bg-gradient-to-br from-orange-50 via-amber-50/40 to-white border border-orange-100">
              <div className="flex items-center gap-1.5 text-xs font-bold text-[#E85A26]">
                ...
                <span>FoodFlow Rewards</span>
              </div>
              <div className="mt-1 flex items-center justify-between text-xs">
                <span className="font-bold text-gray-800">450 Points</span>
                <span className="text-[10px] font-bold text-[#FF6B35]">Redeem</span>
              </div>
            </div>
          )}
